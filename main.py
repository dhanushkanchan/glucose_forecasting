...
full_data_patients = [1014,1131,1159,1242,1259,1275,1348,1391,1408,1420,1431,1436,1443,1466,1474,1489,1346]
...
for i in range(full_data_patients.__len__()):
	...
	x_train, y_train, x_val, y_val = Dataset(df).clean_df_2(columns_list=['timestamp', 'glucose', 'carbs', 'steps']).series_to_supervised()
	print(x_train.shape)
	if i == 0:
...
print(master_x_train.shape, master_y_train.shape, master_x_val.shape, master_y_val.shape)
# print(x_train[:5], y_train[:5], x_val[:5], y_val[:5])


# Load and train model
forecasting_model = Model().train(master_x_train, master_y_train, master_x_val, master_y_val, load_if_saved=False, name="multi_patient_carbs_steps_unscaled", train_again=True)

# Predict

#model = tf.keras.models.load_model('model.h5')
y_predicted  = forecasting_model.predict(master_x_val[:20*cfg.TRAIN.BATCH_SIZE], master_y_val[:20*cfg.TRAIN.BATCH_SIZE])
# print(y_predicted)